"""
HTML renderer for network graph visualization.
...
<head>
<meta charset="utf-8">
<title>SimpleReconSubdomain — Network Map ({targets})</title>
...
<div id="wrap">
  <aside id="side">
    ...
    <h2>Nodes</h2>
    <ul id="legend-nodes">
      <li><span class="dot" style="background:#1976d2"></span>domain<span class="count">{c_domains}</span></li>
      <li><span class="dot" style="background:#9e9e9e"></span>subdomain<span class="count">{c_subdomains}</span></li>
      <li><span class="dot" style="background:#00897b"></span>ip<span class="count">{c_ips}</span></li>
      <li><span class="dot" style="background:#fbc02d"></span>cloud<span class="count">{c_clouds}</span></li>
      <li><span class="dot" style="background:#7b1fa2"></span>cname<span class="count">{c_cnames}</span></li>
      <li><span class="dot" style="background:#e91e63"></span>tld_variant<span class="count">{c_tld_variants}</span></li>
    </ul>

    <h2>Subdomain status</h2>
    <ul>
    ...
    <h2>Edges</h2>
    <ul>
      <li>has_subdomain<span class="count">{c_edges}</span></li>
    </ul>
  </aside>
  <main id="net"></main>
  <div id="info"></div>
...
const nodes = new vis.DataSet(GRAPH.nodes.map(n => ({{
  ...
  shape: ({{ domain: 'star', subdomain: 'dot', ip: 'square', cloud: 'diamond', cname: 'triangle', tld_variant: 'triangleDown' }})[n.type] || 'dot',
  size: n.type === 'domain' ? 26 : (n.type === 'subdomain' ? 14 : 12),
...
def render_html(results: list[dict]) -> str:
    ...
    target_names = [r.get('domain', '') for r in results if r.get('domain')]
    targets_html = '<br>'.join(html.escape(t) for t in target_names) or '—'

    return _TEMPLATE.format(
        targets=html.escape(', '.join(target_names) or 'network map'),
        targets_html=targets_html,
        ts=datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
        c_domains=stats.get('domains', 0),
        c_subdomains=stats.get('subdomains', 0),
        c_ips=stats.get('ips', 0),
        c_clouds=stats.get('clouds', 0),
        c_cnames=stats.get('cnames', 0),
        c_tld_variants=stats.get('tld_variants', 0),
